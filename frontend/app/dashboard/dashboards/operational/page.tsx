...
import { devices } from "../../../dashboard/devices/data";
import { alerts } from "../../../dashboard/alerts/data";
...
export default function OperationalDashboardPage() {
  ...
  const activeAlerts = alerts.filter(
    (a) => a.state === "active" && !acknowledgedIds.has(a.id),
  );
  ...
  const avgReduction = (devices.filter((d) => d.reduction > 0).reduce((s, d) => s + d.reduction, 0) / devices.filter((d) => d.reduction > 0).length).toFixed(1);
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400">
            Live monitoring · {devices.length} registered devices · Last refreshed just now
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <MetricCard title="Avg CO Input"        value={String(avgInput)}     unit="ppm" accent="red"    />
        <MetricCard title="Avg After Purification" value={String(avgOutput)} unit="ppm" accent="green"  />
        ...
        {activeAlerts.length === 0 ? (
          ...
          <div className="divide-y divide-slate-100 dark:divide-slate-700">
            {activeAlerts.map((alert) => (
              <div key={alert.id} className={`flex items-start gap-4 px-6 py-4 border-l-4 ${ALERT_ROW[alert.severity]} ${
                alert.severity === "critical" ? "border-l-red-500" :
                alert.severity === "warning"  ? "border-l-yellow-500" : "border-l-blue-400"
              }`}>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[11px] text-slate-400 mt-1">{alert.device} · {alert.triggeredAt}</p>
                </div>
                <button
