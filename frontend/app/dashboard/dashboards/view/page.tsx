...
function airQualityLabel(avgInput: number): { label: string; color: string; bg: string; icon: React.ReactNode } {
...
const DEVICE_STATUS_STYLES: Record<string, { dot: string; label: string; text: string }> = {
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400">
            Read-only air quality overview · {devices.length} monitored locations
          </p>
        </div>
        <DarkModeToggle />
      </div>
          ...
          <div key={label} className={`bg-white dark:bg-slate-800 rounded-2xl border ${border} p-5 flex items-center gap-4`}>
            ...
            <div>
              <p className={`text-2xl font-extrabold ${color}`}>
                {value} <span className="text-base font-normal text-slate-400">/ {total}</span>
              </p>
                  ...
                  <>
                    ...
                    <div>
                      <div className="flex justify-between text-[11px] text-slate-400 mb-1">
                        ...
                        <span className={device.reduction >= 50 ? "text-green-500 font-semibold" : "text-yellow-500 font-semibold"}>
                          {device.reduction}%
                        </span>
                      </div>
